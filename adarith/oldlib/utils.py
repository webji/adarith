...
DEFAULT_SCREEN = (1024, 768)
DEFAULT_CAPTION = 'Ada\'s World'
...
def draw_text(surf, text, size, color, x, y, align="topleft", font=pg.font.match_font('hack')):
...
def check_none(target=None):
    if target == None:
        raise ArgumentException('Should not be None')
...
def check_none_or_empty(lists=None):
    if is_none_or_empty(lists=lists):
        raise ArgumentException('Should not be None or Empty')